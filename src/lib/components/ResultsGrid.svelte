<script lang="ts">
  ...
  let breakdown = $derived(
    metricsFromScores(rounds.flatMap((r) => r.strokeScores)),
  );

  const METRIC_TIPS: Record<string, string> = {
    "Path Accuracy":
      "Focus on tracing closer to the target \u2014 ghost the line before committing.",
    Smoothness:
      "Your strokes are jittery \u2014 try drawing from the shoulder with a single confident motion.",
    "Speed Control":
      "Your stroke speed is inconsistent \u2014 aim for a steady, even pace throughout.",
    "Endpoint Precision":
      "You're overshooting or undershooting \u2014 ghost the endpoints before drawing.",
    Closure:
      "Your shapes aren't closing cleanly \u2014 slow down at the end to meet your start point.",
    Pressure:
      "Your pressure varies too much \u2014 practice maintaining even pressure.",
    Taper: "Work on lifting smoothly at stroke ends for cleaner tapers.",
    Economy:
      "Try to complete each shape in fewer strokes \u2014 commit to confident single motions.",
  };
  ...
  function generateFeedback(items: Breakdown[]): string {
    ...
    if (worst.value >= 70) {
      ...
      return tip
        ? `Solid work. To push higher: ${tip.charAt(0).toLowerCase()}${tip.slice(1)}`
        : "Solid work \u2014 keep practicing.";
    }

    return (
      METRIC_TIPS[worst.label] ??
      "Keep practicing \u2014 focus on one metric at a time."
    );
  }
  ...
  function renderCanvas(
    ...
    let minX = shapeBounds.minX,
      minY = shapeBounds.minY;
    let maxX = shapeBounds.maxX,
    ...
    for (let i = 0; i < round.strokes.length; i++) {
      ...
      if (score) {
        if (plugin.renderScoredStroke)
          plugin.renderScoredStroke(ctx, round.strokes[i], score);
...
<div class="results-overlay">
  ...
  <div class="results-header">
    ...
    <div class="results-stats">
      ...
      <span class="stat-best" style="color: {scoreColor(bestScore())}"
        >Best {bestScore()}</span
      >
      <span class="stat-worst" style="color: {scoreColor(worstScore())}"
        >Worst {worstScore()}</span
      >
    </div>
  </div>
      ...
      {#each breakdown as item}
        <div class="breakdown-item">
          <div class="breakdown-header">
            ...
            <span class="breakdown-value" style="color: {item.color}"
              >{item.value}</span
            >
          </div>
  ...
  <div class="results-grid" bind:this={gridEl}>
    {#each rounds as round, i}
      ...
      >
        <canvas bind:this={canvasRefs[i]} class="thumb-canvas"></canvas>
      ...
      >
        {#each hoveredBreakdown as item}
          <div class="tooltip-row">
            ...
            <span class="tooltip-value" style="color: {item.color}"
              >{item.value}</span
            >
          </div>
        {/each}
  ...
  {#if galleryIndex != null}
    ...
    >
      ...
      <div class="gallery-panel" onclick={(e) => e.stopPropagation()}>
        ...
        <div
          class="gallery-score"
          style="color: {scoreColor(rounds[galleryIndex].shapeScore)}"
        >
          {rounds[galleryIndex].shapeScore}
          <span class="gallery-counter"
            >{galleryIndex + 1} / {rounds.length}</span
          >
        </div>

        <canvas bind:this={galleryCanvas} class="gallery-canvas"></canvas>
            ...
            {#each galleryBreakdown as item}
              <div class="gallery-metric-row">
                ...
                <div class="gallery-metric-bar-bg">
                  <div
                    class="gallery-metric-bar-fill"
                    style="width: {item.value}%; background: {item.color}"
                  ></div>
                </div>
                <span class="gallery-metric-value" style="color: {item.color}"
                  >{item.value}</span
                >
              </div>
            {/each}
        ...
        <div class="gallery-nav">
          ...
          >
          <button class="gallery-nav-btn" onclick={galleryNext} title="Next (→)"
            >›</button
          >
        </div>
      </div>
