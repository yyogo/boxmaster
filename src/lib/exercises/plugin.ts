...
export interface ExercisePlugin {
	...
	renderGuide(ctx: CanvasRenderingContext2D, params: Record<string, unknown>, visibility: GuideVisibility): void;
	renderScaffold?(ctx: CanvasRenderingContext2D, params: Record<string, unknown>): void;

	renderStroke?(ctx: CanvasRenderingContext2D, stroke: Stroke, color: string, baseWidth: number): void;

	renderScoredStroke?(ctx: CanvasRenderingContext2D, stroke: Stroke, score: StrokeScore): void;
	...
	renderReview?(ctx: CanvasRenderingContext2D, params: Record<string, unknown>, strokes: Stroke[]): void;
...
export function buildMetricScore(points: StrokePoint[], config: MetricConfig): StrokeScore {
	...
	if (config.endpointAccuracy) {
		score.endpointAccuracy = scoreEndpointAccuracy(
			...
			config.endpointAccuracy.startWeight != null ? { startWeight: config.endpointAccuracy.startWeight } : undefined,
		);
	}
...
export function getStrokePoints(stroke: Stroke): StrokePoint[] {
	return stroke.smoothedPoints.length > 0 ? stroke.smoothedPoints : stroke.rawPoints;
}
